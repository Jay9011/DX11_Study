...
uint Address;
SamplerState Sampler_Adress_Wrap
{
    AddressU = Wrap;
    AddressV = Wrap;
};
SamplerState Sampler_Adress_Mirror
{
    AddressU = Mirror;
    AddressV = Mirror;
};
SamplerState Sampler_Adress_Clamp
{
    AddressU = Clamp;
    AddressV = Clamp;
};
SamplerState Sampler_Adress_Border
{
    AddressU = Border;
    AddressV = Border;
...
{
    if (Address == 0)
        return Map.Sample(Sampler_Adress_Wrap, input.Uv);
    
    if (Address == 1)
        return Map.Sample(Sampler_Adress_Mirror, input.Uv);
    
    if (Address == 2)
        return Map.Sample(Sampler_Adress_Clamp, input.Uv);
    
    if (Address == 3)
        return Map.Sample(Sampler_Adress_Border, input.Uv);
