...
/// <param name="instanceId">수정할 Instance 번호</param>
/// <param name="boneIndex">수정할 Bone 번호</param>
/// <param name="transform">수정할 값</param>
...
void ModelRender::CreateTexture()
{
	...
	desc.Usage = D3D11_USAGE_DYNAMIC;	// CPU에서 쓰고 GPU에서 읽기 위해서 Dynamic을 사용
	...
	for (UINT i = 0; i < MAX_MODEL_INSTANCE; i++)
	{
		for (UINT b = 0; b < model->BoneCount(); b++)
		{
			...
			if (parentIndex < 0)
				D3DXMatrixIdentity(&parent);	// Root는 Identity로 초기화 합니다.
			else
	...
	subResource.SysMemPitch = MAX_MODEL_TRANSFORMS * sizeof(Matrix);	// 한 줄의 크기
	subResource.SysMemSlicePitch = MAX_MODEL_TRANSFORMS * sizeof(Matrix) * MAX_MODEL_INSTANCE;	// 한 면의 크기
