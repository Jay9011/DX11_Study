...
float Terrain::GetVerticalRaycase(Vector3& position)
{
    UINT x = (UINT)position.x;
    UINT z = (UINT)position.z;
    ...
    if (TRUE == D3DXIntersectTri(&p[0], &p[1], &p[2], &start, &direction, &u, &v, &distance))
        result = p[0] + (p[1] - p[0]) * u + (p[2] - p[0]) * v;

    if (TRUE == D3DXIntersectTri(&p[3], &p[1], &p[2], &start, &direction, &u, &v, &distance))
...
void Terrain::CreateIndexData()
{
    ...
    for (UINT y = 0; y < height - 1; y++)
    {
        for (UINT x = 0; x < width - 1; x++)
        {
            ...
            indices[index++] = width * (y + 1) + x;
            indices[index++] = width * y + x + 1;
            indices[index++] = width * y + x + 1;
            indices[index++] = width * (y + 1) + x;
            indices[index++] = width * (y + 1) + x + 1;
        }
    }
}
